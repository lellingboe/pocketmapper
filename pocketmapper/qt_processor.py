...
class QTProcessor:
    ...
    def __init__(self, query, target, query_pocket_method, target_pocket_method):
        ...
        self._stage = {"stage": "QTProcessor Initialization"}
        self.logger.debug(
    ...
    def main(self):
        ...
        """
        self._stage.update({"stage": "Main QT Processing"})
    ...
    def process_qt_cmdline_input(self, qt_input, pocket_method=None):
        ...
        """
        self._stage.update({"stage": "Processing Query/Target"})
        ...
        if os.path.isfile(qt_input):  # if it's a file, process each line as a separate query/target
            try:
                ...
                self.logger.critical(f"Problem reading the file {qt_input}: {e}", extra=self._stage)
                exit(1)
        else:
    ...
    def determine_struct_type(self, struct_str):
        ...
        if os.path.isfile(struct_str):
            ...
            logging.critical(f"Could not determine structure type for {struct_str}", extra=self._stage)
            exit(1)
    ...
    def default_pocket_method(self, qt_str):
        ...
        logging.debug(f"Determining pocket method for {pocket_info_str} using regex patterns", extra=self._stage)
        if re.match(pisa_regex, pocket_info_str):
            return "pisa"
        elif re.match(chain_chain_regex, pocket_info_str):
            return "passthrough"
        else:
            self.logger.critical(f"Could not determine pocket method for {pocket_info_str}", extra=self._stage)
            exit(1)
