...
class PocketMapper():
    def __init__(self, job_file=None, log_file=None, verbose=False, debug=False):
        #Setting up logger
        logger = logging.getLogger("pocketmapper")
    ...
    def search(self, file, caching=False):
        ...
        query_df["pdb_domain"] = query_df.apply(lambda x:x.interaction_pdb + "_" + x.domain_chain, axis=1)
        query_df["pdb_domain_motif"] = query_df.apply(lambda x:x.pdb_domain + "_" + x.motif_chain, axis=1)
        ...
        query_df["structure_found"] = query_df["interaction_pdb"].map(status["structure_found"])

        # Diving the structure files into 
        logging.info("Dividing mmCIF structures...")
        status["divided_struct"] = lib.pdb_preprocessing(
            queries=query_df.query("structure_found").iloc[:,:3].itertuples(index=False),
        ...
        query_df["divided_struct"] = query_df["pdb_domain_motif"].map(status["divided_struct"]).fillna(False)

        # regex of all interactionPdb_domainChain string to match the files for foldseek comparison
        fs_file_regex =  "(" + ")|(".join(query_df.query("divided_struct")["pdb_domain"].unique()) + ")"
        ...
        subprocess.run(
            [
                "foldseek",
                "easy-search",
                job_data["domain_dir"],  # query structure or folder of structure
                job_data["domain_dir"],  # target structure or folder of structure
        ...
        pockets, problem_atoms, problem_residues = lib.calculate_pockets(
            queries=query_df.query("divided_struct").iloc[:,:3].itertuples(index=False),
            motif_dir=job_data["motif_dir"],
        ...
        if os.path.exists(p_c_path):
            ...
            alignment_df = pd.read_csv(job_data["foldseek_path"], sep="\t", engine="c")
            pockets_df = lib.compare_pockets(alignment_df, pockets, blosum_path=blosum_path)#, alphafold=ALPHAFOLD, alphafold_dir=ALPHAFOLD_DIR)
