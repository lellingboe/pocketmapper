...
class PisaDownloader:
    ...
    def download_missing_summaries(self, pdb_codes, summary_dir):
        ...
        """
        logging.info(f"Downloading {len(pdb_codes)} PISA summaries", extra=self.stage)
        success = []
        failure = []
        ...
        if len(failure) > 0:
            logging.warning(f"Failed to download {len(failure)} summaries", extra=self.stage)
    ...
    def parse_summaries(self, pdb_codes, summary_dir):
        ...
        """
        logging.info(f"Parsing {len(pdb_codes)} PISA summaries", extra=self.stage)
        ...
        for pdb_code in tqdm(pdb_codes):
            try:
                ...
                logging.debug(f"Issue parsing summary for {pdb_code}: ({e})", extra=self.stage)
                failure.append(pdb_code)
        if len(failure) > 0:
            logging.warning(f"Failed to parse {len(failure)} PISA summaries", extra=self.stage)
    ...
    def download_missing_assemblies(self, asm_dict, asm_dir):
        ...
        """
        logging.info(f"Downloading {sum(len(v) for v in asm_dict.values())} PISA assemblies", extra=self.stage)
        failure = []
        ...
        if len(failure) > 0:
            logging.warning(f"Failed to download {len(failure)} assemblies", extra=self.stage)
        return success, failure
    ...
    def parse_assemblies(self, asm_dict, asm_dir, interface_dir):
        ...
        for pdb_code, assemblies in tqdm(asm_dict.items()):
            ...
            for asm_id in assemblies:
                try:
                    ...
                    if len(data.keys()) != 1:
                        logging.debug(f"More than one entry in assembly for {pdb_code}_{asm_id}", extra=self.stage)
                    ...
                    logging.debug(f"Unexpected error parsing assembly for {pdb_code}_{asm_id}: ({e})", extra=self.stage)
        ...
        if len(failure) > 0:
            logging.warning(f"Failed to parse {len(failure)} PISA assemblies", extra=self.stage)
        return failure
