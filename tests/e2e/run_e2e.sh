...
test_2|human_domains|rows|Mixed batch file (PDB, local mmCIF, AlphaFold) vs human domains|testfile.txt human_domains --foldseek
test_3|core|rows|Batch file vs batch file, PISA interfaces both sides|pdb_pisa_in.txt pdb_pisa_in.txt --foldseek
test_4|human_domains|rows|Single PISA interface vs human domains|4Q5J:B_F human_domains --foldseek
test_5|human_domains|rows|AlphaFold passthrough residues vs human domains|P06493:A:160,161,162,163,164,165 human_domains --foldseek
test_6|core|rows|Chain-ID case sensitivity (4DX9 a_b vs A_B)|4DX9.txt 4DX9.txt --foldseek
test_7|core|ok|PISA interface vs single-residue AlphaFold pocket (human CDK2)|4Q5J:B_F P24941:A:160 --foldseek
test_8|core|ok|PISA interface vs single-residue AlphaFold pocket (mouse ortholog)|4Q5J:B_F P97377:A:160 --foldseek
test_9|core|rows|AlphaFold passthrough vs AlphaFold passthrough|P06493:A:160,161,162,163,164,165 P24941:A:160,161,162,163,164,165 --foldseek
test_10|needs-pdb-fsdb slow|rows|PISA interface vs a local Foldseek PDB database|4Q5J:B_F @PDB_FSDB@ --target_pocket_method foldseek_db --foldseek
...
Environment:
  ...
  POCKETMAPPER_PDB_FSDB   Path to a prebuilt Foldseek PDB database. Required
                          for test_10, which is skipped when unset.
...
Notes:
  ...
  * test_13 downloads the full PDB Foldseek database (tens of GB) and is
...
Examples:
  ...
  $(basename "$0") test_1 test_3              # two specific cases
USAGE
}
